...
def main():
    ...
    if args.llm == args.llm2:
        exit()
    if args.llm != "gemma" and args.llm2 != "gemma":
        exit()
    ...
    transform_layer = torch.nn.Linear(args.in_dim_2, args.in_dim_1).to(device)
    optimizer = torch.optim.Adam([{'params': classifier.parameters()},
                                    {'params': transform_layer.parameters()}],
    ...
    max_val_1 = 0
    res = []
    indices = list(range(int(0.2*len(target_1))))
    for epoch in range(args.epochs):
        ...
        for st in range(0, int(0.2*len(target_1)), args.batch_size):
            ...
            tmp_indices_1 = indices[st: ed]
            tmp_indices_2 = indices[st: ed]
            view_1, output_1 = classifier(input_data_1)
            view_2, output_2 = classifier(transform_layer(input_data_2))
        ...
        with torch.no_grad():
            ...
            output_2 = classifier(transform_layer(input_data_2))[1].detach()
    ...
    np.save("./log_DA/res_transfer_v1_distinct_dim_"+args.kernel+"_"+str(args.learning_rate)+"_"+args.dataset+"_"+args.llm+"_"+args.llm2+".npy", res)
